% This script calculates apparent steady-state erosion rates for 10Be, 14C,
% and 26Al using the Nforward_steadystate function, with optional soil
% mixing in the upper layer. It uses fminbnd to match measured and
% calculated concentration and searches in logarithmic space to improve
% performance. 
% Richard Ott, 2026
...
for i = 1:n
    if Nlogical(i,1)
        logE = fminbnd(@(x) steady_state_misfit(x, Nobs(i,1), i, 1, sp, consts, zm), log10(emin), log10(emax));
        Erate10(i) = 10.^logE;

        logEhigh = fminbnd(@(x) steady_state_misfit(x, Nobs(i,1) - dNobs(i,1), i, 1, sp, consts, zm), log10(emin), log10(emax));
        logElow = fminbnd(@(x) steady_state_misfit(x, Nobs(i,1) + dNobs(i,1), i, 1, sp, consts, zm), log10(emin), log10(emax));
...
for i = 1:n
    if Nlogical(i,2)
        logE = fminbnd(@(x) steady_state_misfit(x, Nobs(i,2), i, 2, sp, consts, zm), log10(emin), log10(emax));
        Erate14(i) = 10.^logE;

        logEhigh = fminbnd(@(x) steady_state_misfit(x, Nobs(i,2) - dNobs(i,2), i, 2, sp, consts, zm), log10(emin), log10(emax));
        logElow = fminbnd(@(x) steady_state_misfit(x, Nobs(i,2) + dNobs(i,2), i, 2, sp, consts, zm), log10(emin), log10(emax));
...
for i = 1:n
    if Nlogical(i,3)
        logE = fminbnd(@(x) steady_state_misfit(x, Nobs(i,3), i, 3, sp, consts, zm), log10(emin), log10(emax));
        Erate26(i) = 10.^logE;

        logEhigh = fminbnd(@(x) steady_state_misfit(x, Nobs(i,3) - dNobs(i,3), i, 3, sp, consts, zm), log10(emin), log10(emax));
        logElow = fminbnd(@(x) steady_state_misfit(x, Nobs(i,3) + dNobs(i,3), i, 3, sp, consts, zm), log10(emin), log10(emax));
...
vars = {'Sample', 'zm_cm', 'E10_mmka', 'dE10_mmka', 'E14_mmka', 'dE14_mmka', 'E26_mmka', 'dE26_mmka'};
out_table = table(samp_names, repmat(zm,n,1), Erate10, dErate10, Erate14, dErate14, Erate26, dErate26, 'VariableNames', vars);
disp(out_table)
...
if has14 || has26
    ...
    if has14
        ...
        title(sprintf('^{14}C vs ^{10}Be, z_m = %.0f cm', zm), 'Interpreter', 'tex')
        panelIdx = panelIdx + 1;
    ...
    if has26
        ...
        title(sprintf('^{26}Al vs ^{10}Be, z_m = %.0f cm', zm), 'Interpreter', 'tex')
    end
end


function err = steady_state_misfit(logE, Ntarget, sampleIdx, nuclideIdx, sp, consts, zm)
...
Nss = Nforward_steadystate(10.^logE, sp, consts, zm, Nlogical);

if ~isfinite(Nss) || any(Nss <= 0)
    err = inf;
else
